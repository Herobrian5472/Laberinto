...
#cambio minimo 2: inicializar la variable para que la funcion no falle
_personaje = None

def MOSTRAR_MARCADOR():
    if _personaje is None:
        return
...
#cambio minimo 1: asegurar la ruta del archivo relativo al script
RUTA_BASE = os.path.dirname(__file__)
RUTA_LABERINTO = os.path.join(RUTA_BASE, "Laberinto.txt")

XX = 0
YY = 0
with open(RUTA_LABERINTO, "r") as archivo:
    for linea in archivo:
        #cambio 3 opcional: strip() para ignorar '\n' (mantiene la logica del libro)
        for sprite in linea.strip():
            if sprite == 'M':
                _pared = PARED(XX, YY)
